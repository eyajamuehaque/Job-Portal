...
// Fetch complaints submitted by this employer
$sql = "SELECT c.*, u.name as subject_name, u.role as subject_role
        FROM complaints c
...
<div class="container">
    ...
    <div style="background: #fff3cd; padding: 15px; border-radius: 5px; margin-bottom: 30px; border: 1px solid #ffeeba;">
        <p style="margin: 0; color: #856404;">If you've encountered abusive conduct from a Job Seeker or unauthorized actions from a Recruiter, please report it here for Admin investigation.</p>
        
        <form method="POST" action="complaints.php" style="margin-top: 15px;">
            <input type="hidden" name="action" value="submit">
            
            <div class="form-group">
                <label>Subject ID (Optional - Seeker ID or Recruiter ID)</label>
                <input type="number" name="subject_id" placeholder="Leave blank if general platform issue">
            </div>
            
            <div class="form-group">
                <label>Detailed Description</label>
                <textarea name="description" rows="5" required placeholder="Describe the issue in detail. If reporting a user, please explain what happened."></textarea>
            </div>
                ...
                <div class="complaint-card">
                    ...
                    <?php if ($c['subject_name']): ?>
                        <p style="margin: 0 0 10px 0; font-size: 14px; color: #666;">
                            <strong>Regarding:</strong> <?= htmlspecialchars($c['subject_name']) ?> 
                            (<?= htmlspecialchars(ucfirst($c['subject_role'])) ?> ID: <?= $c['subject_id'] ?>)
                        </p>
                    <?php endif; ?>
